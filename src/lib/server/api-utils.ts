...
export async function scheduleTask(authorisation: string, taskData: TaskData) {
	if (taskData.dateScheduled && taskData.estimatedDuration) {
		fetch('https://www.googleapis.com/calendar/v3/calendars/primary/events',
		{
			...
			body: JSON.stringify({
                ...
                start: {
                    dateTime: taskData.dateScheduled,
                },
                end: {
                    dateTime: new Date(new Date(taskData.dateScheduled) )
                }
		    }),
	});
    }
